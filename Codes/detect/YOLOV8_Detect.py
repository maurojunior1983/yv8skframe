...
# ------------------------
OUT_DIR    = r"C:/Fr_DataSet_S_K_Frame/YOLOv8_Detect_results"
...
out1 = os.path.join(OUT_DIR, "comparison_start_end_yolov8.xlsx")
out2 = os.path.join(OUT_DIR, "frame_fp_fn_yolov8_adjusted.xlsx")
df1.to_excel(out1, index=False)
print(f"✅ Planilha #1 salva em: {out1}\n")

# ------------------------
# Exportar Planilha 2: FP/FN por borda (igual lógica anterior)
# ------------------------
rows2 = []
for vf, info in results.items():
    real = meta_df[meta_df["video_path"].str.contains(vf, na=False)]
